...
from utils.pdf_utils import build_branded_pdf, p, h2, title, spacer
def render():
    ...
    filename = f"/mnt/data/value_charter_{datetime.now().strftime('%Y%m%d')}.pdf"
    flow = [
        title("價值觀 × 行動準則"),
        spacer(6),
        h2("探索重點"),
        p(f"優先照顧：{', '.join(care) if care else '未選'}"),
        p(f"重要原則：{', '.join(principles) if principles else '未選'}"),
        p(f"傳承方式：{', '.join(ways) if ways else '未選'}"),
        spacer(6),
        h2("建議家規 × 資金規則（示意）"),
    ] + [p(f"- {b}") for b in bullets]
    build_branded_pdf(filename, flow)
    with open(filename, "rb") as f:
        st.download_button("⬇️ 下載價值觀 PDF", data=f.read(), file_name=filename.split("/")[-1], mime="application/pdf")
