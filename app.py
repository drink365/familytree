...
# app.py（自研版 v7.2）
# 修正彙整：Graphviz 配偶中點、PyVis options 純 JSON、st.rerun、parents 安全判斷、移除下載 DOT 按鈕
...
def build_graphviz(db: DB) -> Digraph:
    ...
    dot.attr(rankdir=\"TB\", splines=\"ortho\", nodesep=\"0.9\", ranksep=\"1.2\", compound=\"true\")
    ...
    for m in db.marriages.values():
        ...
        for c in kids:
            dot.edge(uid, c, dir=\"none\", tailport=\"s\", headport=\"n\")
    ...
    for child, parents in parents_of.items():
        if not parents:
            continue
        if len(parents) == 1:
            dot.edge(parents[0], child, dir=\"none\", tailport=\"s\", headport=\"n\")
