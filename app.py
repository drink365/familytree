...
(function(){
  ...
  const ELBOW = 24;                       // 重要：子女連線在婚點附近的短水平段長度
      ...
      Object.values(doc.unions).forEach(u=>{
        ...
        if(kids.length>0){
          ...
          kids.forEach(cid=>{
            ...
            if(!nc) return;
            const childTop = nc.y;
            // 本地 busY：位於子女頂上方一小段，並加上各婚姻的 lane offset
            const busY = childTop - (CHILD_TOP_GAP + 6) + offBase;
            const cx = nc.x + NODE_W/2;
            const dir = (cx >= midX) ? 1 : -1;  // 子女在婚點右側或左側

            // 重要：只在婚點附近畫一小段水平（ELBOW），避免跨過其他婚姻
