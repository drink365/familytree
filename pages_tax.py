...
def render():
    ...
    total_deductions = min(funeral, 1_380_000) + spouse_ded + basic_ex + dep_children * 560_000 + asc_count * 1_380_000
    ...
    st.write({"贈與總額": f"{gift_total:,}","免稅額": f"{annual_ex:,}","課稅基礎": f"{gift_taxable:,}","適用稅率": f"{g_result['rate']}%","速算扣除": f"{g_result['quick']:,}","預估應納稅額": f"{g_result['tax']:,}"})
    flow2 = [
