...
def _new_id(prefix):
    ...
    v = st.session_state[k]; st.session_state[k]=v+1
...
def render():
    ...
    with st.expander("① 人物管理", expanded=True):
        ...
        if t["persons"]:
            pid_del = st.selectbox("選擇人物以刪除（可選）", [""]+list(t["persons"].keys()), format_func=lambda x: x if not x else f'{x}｜{t["persons"][x]["name"]}')
    ...
    with st.expander("② 婚姻關係", expanded=True):
        ...
        if t["marriages"]:
            ...
            if c3.button("👶 新增子女", key="btn_add_child"):
                if not child: st.warning("請選擇子女")
                else:
                    t["marriages"][mid]["children"].append(child); t["child_types"].setdefault(mid, {})[child]=ctype; st.success("已新增子女")
    ...
    with st.expander("④ 匯入 / 匯出", expanded=False):
        ...
        col1.download_button("⬇️ 下載 JSON", data=json.dumps(t, ensure_ascii=False, indent=2), file_name="family_tree.json", mime="application/json", key="btn_dl_json")
        upl = col2.file_uploader("或：上傳 JSON 匯入", type=["json"])
        if upl is not None:
            try:
                data = json.loads(upl.read().decode("utf-8"))
                assert "persons" in data and "marriages" in data
