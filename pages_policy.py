...
def render():
    ...
    filename = f"/mnt/data/policy_strategy_{datetime.now().strftime('%Y%m%d')}.pdf"
    flow = [
        title("保單策略摘要"),
        spacer(6),
        p(f"策略目標：{goal}"),
    ...
    with open(filename, "rb") as f:
        st.download_button("⬇️ 下載保單策略 PDF", data=f.read(), file_name=filename.split("/")[-1], mime="application/pdf")
