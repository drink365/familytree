...
def build_report_html(is_internal: bool = True):
    ...
    assets = st.session_state.assets; plan = st.session_state.plan
    members = member_list(); evs = event_list()
    ...
    table{width:100%;border-collapse:collapse;margin:8px 0 16px 0;} th,td{border:1px solid #e5e7eb;padding:8px;text-align:left;}
    ...
    fam_html = ""
    if members:
        fam_html += "<h2>家族成員摘要</h2><ul>" + "".join([f"<li>{m['name']}（{m['gender'] or '性別未填'}）</li>" for m in members]) + "</ul>"
    if evs:
        def mname(mid): 
            for m in members:
                if m['id']==mid: return m['name']
            return '?'
        fam_html += "<h2>近期重大事件</h2><ul>" + "".join([f"<li>{mname(e['member_id'])}：{e['type']}｜{e['date'] or ''}</li>" for e in evs[:5]]) + "</ul>"

    if is_internal:
        rows = "".join([f"<tr><td>{k}</td><td>{fmt_money(v)} 萬</td></tr>" for k,v in assets.items()])
        assets_html = f"<h2>資產彙總</h2><table><thead><tr><th>項目</th><th>金額</th></tr></thead><tbody>{rows}</tbody></table><div class='muted'>單位：萬元；此區僅供規劃模擬。</div>"
    else:
        total = sum(assets.values())
        assets_html = f"<h2>資產概覽</h2><p>總額：約 <strong>{fmt_money(total)} 萬</strong>（明細留存於內部版）。</p><div class='muted'>分享版僅呈現彙總資訊，避免曝露個別資產細節。</div>"
    plan_rows = "".join([f"<tr><td>{k}</td><td>{v}%</td></tr>" for k,v in plan.items()])
    plan_html = f"<h2>策略配置（比例）</h2><table><thead><tr><th>項目</th><th>比例</th></tr></thead><tbody>{plan_rows}</tbody></table>"
    if est:
        gap = max(0, est['est_tax'] - est['cash_liq'])
        if is_internal:
            metrics_html = f"<h2>重點指標</h2><ul><li>估算遺產稅：{fmt_money(est['est_tax'])} 元</li><li>估算可動用現金：{fmt_money(est['cash_liq'])} 元</li><li>流動性缺口：<strong>{fmt_money(gap)} 元</strong></li></ul>"
        else:
            def approx(n):
                base = (int(n) // 500000) * 500000
                return f"約 {base:,}+ 元"
            metrics_html = f"<h2>重點指標（摘要）</h2><ul><li>估算遺產稅：{approx(est['est_tax'])}</li><li>估算可動用現金：{approx(est['cash_liq'])}</li><li>流動性缺口：<strong>{approx(gap)}</strong></li></ul><div class='muted'>分享版呈現約略值以降低敏感度。</div>"
    else:
        metrics_html = "<p class='muted'>尚未完成資產與策略輸入，無法生成重點指標。</p>"
    disclaimer = "<p class='muted'>本報告為教育與模擬用途，非正式法律／稅務建議；實際規劃請與專業團隊進一步確認。</p>"
    return f"<!doctype html><html><head><meta charset='utf-8'>{style}</head><body>{head}{fam_html}{assets_html}{plan_html}{metrics_html}{disclaimer}</body></html>"
...
def build_report_pdf(is_internal: bool = True) -> bytes:
    ...
    est = current_gap_estimate(); fam = st.session_state.family_name or '未命名家族'
    story = [Paragraph("家族傳承初診斷報告", h1), Spacer(1,8),
             Paragraph(("內部版（完整）" if is_internal else "分享版（摘要）"), body), Spacer(1,4),
             Paragraph(f"家族：{fam}", body), Paragraph(f"時間：{datetime.now(TZ).strftime('%Y-%m-%d %H:%M')}", body), Spacer(1,12)]

    members = member_list(); evs = event_list()
    if members:
        story += [Paragraph("家族成員摘要", h2)]
        data = [["姓名","性別","出生","過世"]] + [[m["name"], m["gender"], m["birth"], m["death"]] for m in members]
        from reportlab.platypus import Table
        t = Table(data, hAlign="LEFT"); t.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,colors.grey),('BACKGROUND',(0,0),(-1,0),colors.HexColor("#f1f5f9"))]))
        story += [t, Spacer(1,8)]
    if evs:
        story += [Paragraph("近期重大事件", h2)]
        def mname(mid):
            for m in members:
                if m['id']==mid: return m['name']
            return '?'
        for e in evs[:5]:
            story += [Paragraph(f"• {mname(e['member_id'])}：{e['type']}｜{e['date'] or ''}", body)]
        story += [Spacer(1,8)]

    assets = st.session_state.assets
    if is_internal:
        story += [Paragraph("資產彙總", h2)]
        data = [["項目","金額（萬）"]] + [[k, f"{int(v):,}"] for k,v in assets.items()]
        from reportlab.platypus import Table
        t = Table(data, hAlign="LEFT"); t.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,colors.grey),('BACKGROUND',(0,0),(-1,0),colors.HexColor("#f1f5f9"))])); story += [t, Spacer(1,8)]
    else:
        total = sum(assets.values()); story += [Paragraph("資產概覽", h2), Paragraph(f"總額：約 {int(total):,} 萬（明細留存於內部版）", body), Spacer(1,8)]

    plan = st.session_state.plan
    story += [Paragraph("策略配置（比例）", h2)]
    data = [["項目","比例（%）"]] + [[k, str(v)] for k,v in plan.items()]
    from reportlab.platypus import Table
    t2 = Table(data, hAlign="LEFT"); t2.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,colors.grey),('BACKGROUND',(0,0),(-1,0),colors.HexColor("#f1f5f9"))])); story += [t2, Spacer(1,8)]

    if est:
        gap = max(0, est['est_tax'] - est['cash_liq']); story += [Paragraph("重點指標", h2)]
        if is_internal:
            items = [f"估算遺產稅：{int(est['est_tax']):,} 元", f"估算可動用現金：{int(est['cash_liq']):,} 元", f"流動性缺口：{int(gap):,} 元"]
        else:
            base = lambda n: (int(n)//500000)*500000
            items = [f"估算遺產稅：約 {base(est['est_tax']):,}+ 元", f"估算可動用現金：約 {base(est['cash_liq']):,}+ 元", f"流動性缺口：約 {base(gap):,}+ 元"]
        for it in items: story += [Paragraph("• " + it, body)]
        story += [Spacer(1,8)]
    else:
...
with col1:
    ...
    try:
        ...
        st.info("若需 PDF，請確認環境已安裝 `reportlab` 後重試（已在 requirements.txt 列出）。")
